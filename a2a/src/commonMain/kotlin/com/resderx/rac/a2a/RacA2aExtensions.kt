...
/**
 * 通过 A2A 协议调用远端 Agent，将响应归一化为 [AIMessage]（Llm 扩展函数）。
 *
 ...
 *
 * - 作用：创建 [LlmA2aAgent]（将 Llm 的 AI 调用能力适配为 A2A Agent）并包装进 [A2aAgentServer]，
 *   使任何 A2A 兼容的 Client 都能通过 A2A 协议调用 Llm 管理的 AI 供应商
 * - 必要性：Llm 需支持 A2A Agent 角色（Server 端），与 Client 角色（[chatWithA2aAgent]）对称；
 *   本方法封装 LlmA2aAgent + A2aAgentServer 的创建，调用方只需配置 Agent Card 与系统提示
 * - 模块拆分：本扩展函数位于 rac-a2a 模块，避免 core 模块依赖 A2A 协议包
 * - 设计思路：
 *   1. 构造 [AgentCard]：描述 Agent 身份、能力、端点
 *   2. 构造 [LlmA2aAgent]：以当前 Llm 实例为 AI 引擎
