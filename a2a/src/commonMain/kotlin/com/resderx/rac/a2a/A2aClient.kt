...
interface A2aClient {
     ...
     * @return 发送结果——[SendMessageResult.TaskResult] 表示异步任务，
     *   [SendMessageResult.MessageResult] 表示同步直接响应
     */
     ...
     *
     * - 流事件类型：[TaskStatusUpdateEvent]（状态变化）、[TaskArtifactUpdateEvent]（产出物块）、
     *   [SendMessageResult]（初始 Task 或 Message）
 ...
 * 创建 A2A 客户端实例的工厂函数。
 *
 * - 作用：隐藏 [DefaultA2aClient] 实现细节，仅暴露 [A2aClient] 接口
 * - 用法：`val client = A2aClient(config)`
 *
 * @param config 客户端配置
 * @return A2A 客户端实例
 */
fun A2aClient(config: A2aClientConfig): A2aClient = DefaultA2aClient(config)
