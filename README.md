...
| 模块 | Maven 坐标 | 作用 |
| --- | --- | --- |
| core | `com.resderx.rac:core:0.3.0-alpha` | 消息模型、网络层、API 协议客户端、供应商实现、基础 DSL、Agent |
| mcp | `com.resderx.rac:mcp:0.3.0-alpha` | MCP 客户端 + `Llm.chatWithMcp` 扩展函数 |
| acp | `com.resderx.rac:acp:0.3.0-alpha` | ACP 双向支持 + `Llm.chatWithAcpAgent` / `Llm.serveAsAcpAgent` 扩展函数 |
| a2a | `com.resderx.rac:a2a:0.3.0-alpha` | A2A 双向支持 + `Llm.chatWithA2aAgent` / `Llm.serveAsA2aAgent` 扩展函数 |
...
dependencies {
    commonMain {
        implementation("com.resderx.rac:core:0.3.0-alpha")
        // 按需引入协议模块
        implementation("com.resderx.rac:mcp:0.3.0-alpha")
        implementation("com.resderx.rac:acp:0.3.0-alpha")
        implementation("com.resderx.rac:a2a:0.3.0-alpha")
    }
}
```
