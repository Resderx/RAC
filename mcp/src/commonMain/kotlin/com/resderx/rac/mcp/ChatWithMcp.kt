...
/**
 * 带 MCP 工具自动注册与多轮调用的非流式 Chat 调用（RAC 扩展函数）。
 *
 * - 作用：自动从 [McpClient] 发现工具并注入到对话，复用 [RAC.chatWithTools] 的多轮工具调用循环，
 ...
 *   2. 委托 [RAC.chatWithTools]，在 builder 块内先执行调用方的 [block]（用户消息/参数），
 ...
 *   再将工具作为闭包变量传入 [RAC.chatWithTools] 的块内调用 addTools
 ...
 * - 优点：调用方零样板代码即可获得 MCP 工具调用能力；与 [RAC.chatWithTools] 共享循环逻辑，行为一致
 * - 算法/数据结构：委托 [RAC.chatWithTools]，无额外算法
 * - 时间复杂度：O(L + R * (T + N))，L 为 listTools 耗时，R 为循环轮数，T 为每轮工具数，N 为响应解析
 * - 空间复杂度：同 [RAC.chatWithTools]，额外 O(K) 存放 MCP 工具列表
 *
 * @receiver RAC 实例，提供 chatWithTools 能力
 * @param mcpClient 已连接的 MCP 客户端，提供工具发现与调用能力
 * @param maxRounds 最大工具调用循环轮数，默认 10；透传给 [RAC.chatWithTools]
