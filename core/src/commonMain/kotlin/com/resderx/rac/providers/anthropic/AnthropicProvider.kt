package com.resderx.rac.providers.anthropic
 ...
 *
 * - 作用：以 RAC 内置的 Anthropic 默认配置（baseUrl/defaultModel/defaultApiType/anthropic-version）为基准，
 ...
 * - defaultModel 为 "claude-3-5-sonnet-20241022"
 ...
 *
 * @param config 调用方对默认配置的覆盖项集合，所有字段可空，null 表示沿用 Anthropic 默认
 * @return 配置完成的 [ModelProvider]（实为 [SimpleModelProvider]），defaultHeaders 已包含 anthropic-version
 */
fun AnthropicProvider(config: ProviderConfig): ModelProvider = SimpleModelProvider(
    name = "anthropic",
