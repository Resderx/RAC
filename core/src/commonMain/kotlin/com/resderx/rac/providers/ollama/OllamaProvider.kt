package com.resderx.rac.providers.ollama
 ...
 *      （无鉴权）、`defaultModel = "llama3.1"`。此时 RAC.buildHeaders 不会添加
 ...
 *   减少样板代码；本地模式零配置即可使用
 *
 * @param config 供应商配置，null 字段表示沿用 Ollama 默认值（本地模式默认值）
 * @return 构造完成的 [ModelProvider] 实例，name 为 "ollama"
 */
fun OllamaProvider(config: ProviderConfig): ModelProvider = SimpleModelProvider(
    name = "ollama",
