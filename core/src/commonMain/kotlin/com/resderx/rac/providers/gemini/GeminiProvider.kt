package com.resderx.rac.providers.gemini
 ...
 * - 设计思路：使用 Google 官方提供的 OpenAI 兼容端点
 ...
 *   - [ProviderConfig.model] 可切换为其他 Gemini 模型（如 `gemini-1.5-pro`、`gemini-2.0-flash`），
 ...
 *
 * @param config 供应商配置，null 字段表示沿用 Gemini 默认值
 * @return 构造完成的 [ModelProvider] 实例，name 为 "gemini"
 */
fun GeminiProvider(config: ProviderConfig): ModelProvider = SimpleModelProvider(
    name = "gemini",
