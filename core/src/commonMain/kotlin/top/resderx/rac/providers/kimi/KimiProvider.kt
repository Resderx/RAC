...
package top.resderx.rac.providers.kimi
 ...
 *
 * - 作用：创建 Kimi 的 ModelProvider 实例，预设 baseUrl、模型注册表与 API 类型
 * - 必要性：封装 Kimi 的默认配置，调用方仅需提供 apiKey 即可使用
 * - 设计思路：连接配置与模型配置分离——使用 SimpleModelProvider 减少样板；config 字段覆盖连接默认值，
 *   models 为模型名→配置的 Map，为空时自动注册默认模型 "moonshot-v1-8k"（空 ModelConfig）
 ...
 *   （Kimi 需要 Bearer 鉴权，调用方应设置 apiKey）；models 为空时回落到默认模型，保持零配置可用
 *
 * 默认配置：
 * - baseUrl：https://api.moonshot.cn/v1（Moonshot 官方 OpenAI 兼容端点）
 * - 默认模型注册项：moonshot-v1-8k（8k 上下文窗口版本，models 为空时自动注册）
 ...
 *
 * @param config 连接配置覆盖项（apiKey/baseUrl/headers），null 字段表示沿用 Kimi 默认
...
fun KimiProvider(
    ...
    val resolvedModels = models.takeIf { it.isNotEmpty() }
        ?: mapOf("moonshot-v1-8k" to ModelConfig())
