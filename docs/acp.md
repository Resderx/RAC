...
RAC 的 ACP 支持分为以下组件（位于 `acp/` 模块，`com.resderx.rac.acp` 包）：

```
acp/                                          # 独立 Gradle 模块（rac-acp），依赖 rac-core
...
├── RacAcpAgent.kt           # Llm → ACP Agent 适配器（类名 LlmAcpAgent，文件名保留历史前缀）
└── RacAcpExtensions.kt      # Llm.chatWithAcpAgent / Llm.serveAsAcpAgent 扩展函数
```
...
通过 `AcpClient` 连接外部 Agent，再用 `Llm.chatWithAcpAgent` 扩展函数发起会话（`Llm` 实例仅作为命名空间锚点，不直接使用其内部能力）：
...
`serveAsAcpAgent` 将 Llm 的 `chat` 调用封装为 ACP Agent，自动处理协议握手、会话管理、流式更新推送：
...
`LlmAcpAgent` 适配器内部（文件 `RacAcpAgent.kt`，类名 `LlmAcpAgent`）：
...
- LlmAcpAgent 适配器（prompt → chat → update 推送）
