...
RAC 双向支持 [Agent-to-Agent Protocol v1.0](https://a2a-protocol.org/)——既能作为 Client 调用远端 A2A Agent（Google ADK、LangGraph、CrewAI 等），也能将自身 LLM 调用能力封装为 A2A Agent Server 供其他 Client 调用。
...
```kotlin
// 非流式——发送消息并等待结果
val result = client.sendMessage(SendMessageParams(
    message = Message(
        role = Role.USER,
...
// 流式——订阅 SSE 事件流
client.sendStreamingMessage(SendStreamingMessageParams(
...
`A2aAgentServer.dispatchStreaming` 使用 `Channel` 缓冲 handler 推送的更新，确保 `Initial` 事件始终先于 `StatusUpdate` / `ArtifactUpdate` 发射：
